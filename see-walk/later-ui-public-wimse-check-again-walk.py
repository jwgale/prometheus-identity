#!/usr/bin/env python3
"""Later user interface WIMSE Check again against the live public check name.

Drive the same HTTP JSON GET / posts. Check again posts that JSON a second
time. Off-origin pins use POST /well-known-follow then POST /operator-pin.
...
Public artifacts only land under see-walk/later-ui-public-wimse-check-again.
"""
...
STORE = "/tmp/prometheus-later-ui-public-wimse-check-again-a"
WALK = Path("/home/jason/Projects/Prometheus/see-walk/later-ui-public-wimse-check-again")
ISSUING = "http://127.0.0.1:18824"
...
save(
    "a-init-note.txt",
    "init ran on this machine. Secret files stay under /tmp/prometheus-later-ui-public-wimse-check-again-a.\n",
)
...
try:
    ...
    if missing:
        raise SystemExit("GET / is not the later UI with WIMSE Check again. missing=%s content_type=%s" % (missing, content_type))
    ...
    save(
        ...
        "\n".join(
            [
                "issuing GET / is the later user interface with WIMSE Check again",
    ...
    if "/check-svid" not in check_paths or "/check-wimse" not in check_paths:
        raise SystemExit("public well-known must name /check-svid and /check-wimse: %s" % check_paths)
    for write_verb in (
        "/birth",
        "/spawn",
        "/present-svid",
    ...
    def apply_wimse(wimse):
        ...
        save("a-present-wimse-keys.json", {"keys": sorted(wimse.keys())})
        save(
    ...
    save(
        "runtime-check-request-keys.json",
        {
            ...
            "note": "GET / Check and Check again post this WIMSE JSON to the loopback issuing host. That host signs the verifier nonce locally. Secret bytes are not uploaded. The path is not sent to the public name. WIMSE token, digest, and signature fields are not trimmed.",
        },
    )
    ...
    if "accepted a kill" not in reason.lower() and "kill accept" not in reason.lower():
        raise SystemExit("GET / Check again must refuse from accepted kill: %s" % reason)
    ...
    print("OK later-UI WIMSE Check again allow then refuse")
finally:
    host.terminate()
    try:
        host.wait(timeout=5)
    ...
    save(
        "tmp-cleaned.txt",
        "Deleted /tmp/prometheus-later-ui-public-wimse-check-again-a after the walk. issuer.secret is not left under /tmp from this walk.\n",
    )
