#!/usr/bin/env python3
"""Rung 99: later UI remote member-secret path, then public allow then refuse.
...
Public artifacts only land under see-walk/later-ui-member-two-public.
"""
...
STORE = "/tmp/prometheus-later-ui-member-two-public-20260823"
WALK = Path("/home/jason/Projects/Prometheus/see-walk/later-ui-member-two-public")
...
def check_svid_public(presentation_json, certificate_pem, holder_secret_path):
    ...
    return http("POST", "%s/check-svid" % PUBLIC, body, timeout=20)[:2]
...
if "/check-svid" not in well_known_text:
    raise SystemExit("public well-known must name /check-svid")
for write_verb in ("/birth", "/present-svid", "/kill-export", "/runtime-check", "issuer.secret"):
    if write_verb in well_known_text and write_verb != "/kill-export":
        if write_verb in ("/birth", "/present-svid", "/runtime-check", "issuer.secret"):
...
try:
    ...
    (WALK / "presentation.json").write_text(present["presentation_json"] if present["presentation_json"].endswith("\n") else present["presentation_json"] + "\n")
    (WALK / "presentation.json.svid.pem").write_text(present["certificate_pem"] if present["certificate_pem"].endswith("\n") else present["certificate_pem"] + "\n")
    save(
    ...
    status, allow = check_svid_public(present["presentation_json"], present["certificate_pem"], holder_secret_path)
    ...
    if not is_allowed(status, allow):
        if "expir" in reason.lower():
            ...
            reason = reason_of(allow)
    if not is_allowed(status, allow):
        raise SystemExit("public check-svid must allow: HTTP %s %s" % (status, redact(str(allow)[:800])))
    save(
    ...
    status, refused = check_svid_public(present["presentation_json"], present["certificate_pem"], holder_secret_path)
    ...
    print("OK later-ui remote path public allow then refuse")
finally:
    host.terminate()
    try:
        host.wait(timeout=5)
