...
STORE = "/tmp/prometheus-public-add-act-a"
WALK = Path("/home/jason/Projects/Prometheus/see-walk/public-add-act-agent-process")
ISSUING = "http://127.0.0.1:18799"
...
save(
    "a-init-note.txt",
    "init ran on this machine. Secret files stay under /tmp/prometheus-public-add-act-a.\n",
)
...
"""
    (WALK / "hermes-start-add-act-agent-process.sh").write_text(start_script)
    subprocess.check_call(
        SCP
        + [
            str(WALK / "hermes-start-add-act-agent-process.sh"),
            "ubuntu@52.91.253.34:%s/hermes-start-add-act-agent-process.sh" % REMOTE,
        ]
    )
    subprocess.check_call(
        SSH
        + [
            "chmod 755 %s/prometheus %s/hermes-start-add-act-agent-process.sh && chmod 600 %s/holder.secret && chmod 644 %s/presentation.json %s/presentation.json.svid.pem"
    ...
    start = remote("bash %s/hermes-start-add-act-agent-process.sh" % REMOTE, timeout=25)
    ...
    print("CHILD-PRESENT-SVID", status, child_svid if status != 200 else sorted(child_svid.keys()))
    assert status == 200, child_svid
    ...
    save("a-child-present-svid-keys.json", {"keys": sorted(child_svid.keys())})
    subprocess.check_call(
        SCP
        + [
            str(WALK / "child-presentation.json"),
            "ubuntu@52.91.253.34:%s/child-presentation.json" % REMOTE,
        ]
    )
    subprocess.check_call(
        SCP
        + [
            str(WALK / "child-presentation.json.svid.pem"),
            "ubuntu@52.91.253.34:%s/child-presentation.json.svid.pem" % REMOTE,
        ]
    )
    subprocess.check_call(
    ...
    subprocess.check_call(
        SSH
        + [
            "chmod 600 %s/child-holder.secret && chmod 644 %s/child-presentation.json %s/child-presentation.json.svid.pem"
            % (REMOTE, REMOTE, REMOTE)
        ]
    )
    save(
        "hermes-child-copy-note.txt",
        "Copied child presentation.json, laboratory X.509-SVID wrap, and child-holder.secret to Hermes. Did not copy issuer.secret.\n",
    )

    print("SEND FAIL-CLOSED MIX ADD")
    mix_line = (
        "add-act --presentation-json %s/presentation.json --certificate-pem %s/presentation.json.svid.pem "
        "--workload-identity-token %s/presentation.json --content-digest x --signature-input y --signature z"
        % (REMOTE, REMOTE, REMOTE)
    )
    ...
    secret_line = (
        "add-act --presentation-json %s/child-presentation.json --certificate-pem %s/child-presentation.json.svid.pem "
        "--holder-secret-path %s/child-holder.secret" % (REMOTE, REMOTE, REMOTE)
    )
    ...
    print("SEND HONEST ADD-ACT")
    add_line = (
        "add-act --presentation-json %s/child-presentation.json --certificate-pem %s/child-presentation.json.svid.pem "
        '--holder-proof-command "%s/prometheus holder-sign --holder-secret-path %s/child-holder.secret"'
        % (REMOTE, REMOTE, REMOTE, REMOTE)
    )
    send_add = remote("echo '%s' > %s/agent-process.in" % (add_line, REMOTE))
    if send_add.returncode != 0:
        raise SystemExit("failed to send honest add-act: %s" % send_add.stderr)
    ...
    require_same_pid(pid, "after honest add-act")
    save(
    ...
    if "TOOL_RAN_AFTER_ADD" not in tool_after_add:
        raise SystemExit("tool did not run on Hermes after add-act: %s %s" % (stdout_after_add, stderr_after_add))
    require_same_pid(pid, "after add-act ALLOWED")
    save(
        "hermes-agent-process-after-add-summary.json",
        {
            ...
            "held_acts": "parent X.509-SVID + child X.509-SVID",
        },
    )
    ...
    save(
        "SEE.txt",
        """Public add-act durable agent process walk

Date: 22 August 2026.

This folder is for Jason Gale. These files are public records and artifacts. This folder does not contain issuer.secret, biscuit.secret, holder secrets, or member-two secrets. The issuing store lived under /tmp/prometheus-public-add-act-a.

Create Agent Principal ran on the operator machine and bound 127.0.0.1 only. Spawn of a wider child was refused. Spawn wrote a narrower child. The durable agent process ran on Hermes at 52.91.253.34. The verifier is https://check.prestigeworldwide.digital. Hermes held the parent Assertion Act, then the child Assertion Act after add-act, and the holder keys. Hermes did not hold issuer.secret. Hermes is not a second identity store.

prometheus runtime-check agent-process on Hermes started with the parent X.509-SVID Assertion Act, used holder-sign on that machine, and the public check name. The first tool line printed ALLOWED and ran the tool. A mixed on-ramp add-act line printed REFUSED. An add-act line that named a holder secret path printed REFUSED. An honest add-act of the narrower child printed ADDED on the same process identifier. A later tool line printed ALLOWED only after both documented checks allowed, and then ran the tool. After parent Decommission by kill accept, a later tool line on the same process identifier printed REFUSED and did not run the tool. Death wins on the added child by parent cascade. The refuse names accepted kill. The process did not restart.
