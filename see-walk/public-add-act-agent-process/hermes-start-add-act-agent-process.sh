...
rm -f "$REMOTE/agent-process.in" "$REMOTE/agent-process.stdout" "$REMOTE/agent-process.stderr"   "$REMOTE/agent-process.pid" "$REMOTE/fifo-keeper.pid"   "$REMOTE/tool-allow.txt" "$REMOTE/tool-after-add.txt" "$REMOTE/tool-after-refuse.txt"
