...
STORE = "/tmp/prometheus-public-wimse-a"
WALK = Path("/home/jason/Projects/Prometheus/see-walk/public-wimse-agent-process")
ISSUING = "http://127.0.0.1:18797"
...
save(
    "a-init-note.txt",
    "init ran on this machine. Secret files stay under /tmp/prometheus-public-wimse-a.\n",
)
...
"""
    ...
    subprocess.check_call(
        SSH
        + [
            "chmod 755 %s/prometheus %s/hermes-start-wimse-agent-process.sh && chmod 600 %s/holder.secret && chmod 644 %s/presentation.json %s/workload_identity_token %s/content_digest %s/signature_input %s/signature"
    ...
    save(
        ...
        "Copied prometheus binary, presentation.json, WIMSE token, content-digest, HTTP Message Signature, and holder.secret to Hermes /var/lib/prometheus-agent. Did not copy issuer.secret. Did not copy biscuit.secret.\n",
    )
    ...
    save(
        "SEE.txt",
        """Public WIMSE durable agent process walk

Date: 23 August 2026.

This folder is for Jason Gale. These files are public records and artifacts. This folder does not contain issuer.secret, biscuit.secret, holder secrets, or member-two secrets. The issuing store lived under /tmp/prometheus-public-wimse-a.

Create Agent Principal ran on the operator machine and bound 127.0.0.1 only. The WIMSE Assertion Act was minted on that issuing store. The durable agent process ran on Hermes at 52.91.253.34. The verifier is https://check.prestigeworldwide.digital. Hermes held the presentation, the Workload Identity Token, the HTTP Message Signature, and the holder key. Hermes did not hold issuer.secret. Hermes is not a second identity store.

prometheus runtime-check agent-process on Hermes used the documented WIMSE check path, holder-sign on that machine, and the public check name. The process stayed up. One tool line printed ALLOWED and ran the tool. After Decommission by kill accept, a second tool line on the same process identifier printed REFUSED and did not run the tool. Death wins. The refuse names accepted kill. The process did not restart.
