...
rm -f "$REMOTE/agent-process.in" "$REMOTE/agent-process.stdout" "$REMOTE/agent-process.stderr" "$REMOTE/agent-process.pid" "$REMOTE/fifo-keeper.pid" "$REMOTE/tool-allow.txt" "$REMOTE/tool-both.txt" "$REMOTE/tool-named-live.txt" "$REMOTE/tool-named-dead.txt" "$REMOTE/tool-unnamed-after.txt"
