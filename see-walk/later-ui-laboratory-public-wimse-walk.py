#!/usr/bin/env python3
"""Laboratory operator page WIMSE Check against the live public check name.
...
Public artifacts only land under see-walk/later-ui-laboratory-public-wimse.
"""
...
STORE = "/tmp/prometheus-later-ui-laboratory-public-wimse-a"
WALK = Path("/home/jason/Projects/Prometheus/see-walk/later-ui-laboratory-public-wimse")
ISSUING = "http://127.0.0.1:18820"
...
save(
    "a-init-note.txt",
    "init ran on this machine. Secret files stay under /tmp/prometheus-later-ui-laboratory-public-wimse-a.\n",
)
...
try:
    ...
    markers = [
        ...
        "/well-known-follow",
        "/operator-pin",
        "This page does not fetch the public name",
        'id="emit-wimse"',
    ...
    save(
        ...
        "\n".join(
            [
                "issuing GET /laboratory is the laboratory operator page with WIMSE Check again",
    ...
    save(
        "runtime-check-request-keys.json",
        {
            ...
            "note": "GET /laboratory Check and Check again post this WIMSE JSON to the loopback issuing host. That host signs the verifier nonce locally. Secret bytes are not uploaded. The path is not sent to the public name. WIMSE token, digest, and signature fields are not trimmed.",
        },
    )
    ...
    print("OK laboratory WIMSE Check again allow then refuse")
finally:
    host.terminate()
    try:
        host.wait(timeout=5)
    ...
    save(
        "tmp-cleaned.txt",
        "Deleted /tmp/prometheus-later-ui-laboratory-public-wimse-a after the walk. issuer.secret is not left under /tmp from this walk.\n",
    )
