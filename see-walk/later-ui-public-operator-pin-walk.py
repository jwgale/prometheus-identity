#!/usr/bin/env python3
"""Later-UI operator-pin kill-accept against the live public check name.

Drive the same HTTP JSON GET / posts. Off-origin pins use POST /well-known-follow
then POST /operator-pin. Do not hardcode public /kill-accept as the accept path.
Do not spawn AgentProcess. Do not copy issuer.secret onto the public host.
Public artifacts only land under see-walk/later-ui-public-operator-pin.
"""
...
STORE = "/tmp/prometheus-later-ui-public-operator-pin-a"
WALK = Path("/home/jason/Projects/Prometheus/see-walk/later-ui-public-operator-pin")
ISSUING = "http://127.0.0.1:18814"
...
save(
    "a-init-note.txt",
    "init ran on this machine. Secret files stay under /tmp/prometheus-later-ui-public-operator-pin-a.\n",
)
...
try:
    ...
    print("OFF-NAME OPERATOR-PIN 403")
    off_name = {}
    for check_base in (
        ...
        status, payload, _ = http(
            ...
            {"check_base": check_base, "pin": "kill-accept", "body": {}},
            timeout=20,
        )
        require(status, payload, 403, "off-name POST /operator-pin %s" % check_base)
        off_name[check_base] = {
            "http_status": status,
            "result": payload.get("result") if isinstance(payload, dict) else None,
            "reason": payload.get("reason") if isinstance(payload, dict) else payload,
        }
    save("off-name-operator-pin.json", off_name)
    ...
    print("OK operator-pin kill-accept allow then refuse")
finally:
    host.terminate()
    try:
        host.wait(timeout=5)
    ...
    save(
        "tmp-cleaned.txt",
        "Deleted /tmp/prometheus-later-ui-public-operator-pin-a after the walk. issuer.secret is not left under /tmp from this walk.\n",
    )
