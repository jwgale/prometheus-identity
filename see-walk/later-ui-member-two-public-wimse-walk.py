...
Public artifacts only land under see-walk/later-ui-member-two-public-wimse.
...
WALK = Path("/home/jason/Projects/Prometheus/see-walk/later-ui-member-two-public-wimse")
...
try:
    ...
    if missing:
        raise SystemExit("GET / is not the later UI with WIMSE. missing=%s content_type=%s" % (missing, content_type))
    ...
    save(
        ...
        "\n".join(
            [
                "issuing GET / is the later user interface with present-wimse",
