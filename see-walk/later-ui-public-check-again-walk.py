#!/usr/bin/env python3
"""Later user interface Check again against the live public check name.
...
Public artifacts only land under see-walk/later-ui-public-check-again.
"""
...
STORE = "/tmp/prometheus-later-ui-public-check-again-a"
WALK = Path("/home/jason/Projects/Prometheus/see-walk/later-ui-public-check-again")
ISSUING = "http://127.0.0.1:18822"
...
save(
    "a-init-note.txt",
    "init ran on this machine. Secret files stay under /tmp/prometheus-later-ui-public-check-again-a.\n",
)
...
try:
    ...
    if missing:
        raise SystemExit("GET / is not the later UI with Check again. missing=%s content_type=%s" % (missing, content_type))
    ...
    save(
        ...
        "\n".join(
            [
                "issuing GET / is the later user interface with Check again",
    ...
    save(
        "runtime-check-request-keys.json",
        {
            ...
            "note": "GET / Check and Check again post this JSON to the loopback issuing host. That host signs the verifier nonce locally. Secret bytes are not uploaded. The path is not sent to the public name.",
        },
    )
    ...
    print("OK later-UI Check again allow then refuse")
finally:
    host.terminate()
    try:
        host.wait(timeout=5)
    ...
    save(
        "tmp-cleaned.txt",
        "Deleted /tmp/prometheus-later-ui-public-check-again-a after the walk. issuer.secret is not left under /tmp from this walk.\n",
    )
